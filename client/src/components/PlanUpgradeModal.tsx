        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} style={{ background: '#fff', borderRadius: '32px', maxWidth: '900px', width: '100%', padding: '40px', position: 'relative' }}>
                ...
                <div style={{ textAlign: 'center', marginBottom: '40px' }}>
                    <h2 style={{ fontSize: '2.5rem', fontWeight: 800 }}>{t('dashboard.plans.choosePlan')}</h2>
                    <p style={{ color: '#666' }}>{t('dashboard.plans.boostReach')}</p>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '20px' }}>
                    <PlanCard
                        ...
                        features={[
                            t('dashboard.plans.f1'),
                            t('dashboard.plans.f2'),
                            t('dashboard.plans.f3')
                        ]}
                    ...
                    <PlanCard
                        ...
                        icon={<Crown size={24} />}
                        features={[
                            t('dashboard.plans.f4'),
                            t('dashboard.plans.f5'),
                            t('dashboard.plans.f6')
                        ]}
...
function PlanCard({ name, price, color, icon, features, onSelect, loading, t }: PlanCardProps) {
    return (
        <div style={{ border: `2px solid ${color}20`, padding: '30px', borderRadius: '24px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '2.5rem', fontWeight: 800, marginBottom: '20px' }}>{price} <span style={{ fontSize: '1rem', color: '#999' }}>{t('dashboard.plans.perMonth')}</span></div>
            ...
            >
                {loading ? <Loader2 size={18} className="animate-spin" /> : `${t('dashboard.plans.subscribe')} ${name}`}
            </button>
        </div>
    );
}
