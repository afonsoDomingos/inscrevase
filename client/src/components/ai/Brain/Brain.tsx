...
export default function Brain() {
    ...
    // Dispatch visibility change for coordination with other UI elements
    useEffect(() => {
        window.dispatchEvent(new CustomEvent('brain-visibility-change', {
...
Experimenta agora e leva os teus eventos para o próximo nível.”`;
        ...
        if (lowerTranscript.includes('quem te criou') || lowerTranscript.includes('quem é o teu criador') || lowerTranscript.includes('quem te fez') || lowerTranscript.includes('afonso domingos') || lowerTranscript.includes('quem é o afonso') || lowerTranscript.includes('quem e o afonso')) {
            const creatorInfo = "Fui concebido e orquestrado por **Afonso Domingos**, Fundador da Inscreva-se e da RPA Moçambique. A redirecioná-lo para o perfil do Mestre Supremo.";
            setChatHistory(prev => [...prev, { role: 'ai', text: creatorInfo }]);
            speak("Fui concebido e orquestrado por Afonso Domingos, Fundador da Inscreva-se. A abrir o perfil do Mestre Supremo.");
            setIsThinking(false);
            setTimeout(() => { router.push('/equipe/afonso-domingos'); }, 2000);
            return;
        }
                        ...
                        >
                            ...
                            >
                                ...
                                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: isMobile ? '4px' : '12px', overflow: 'hidden' }}>
                                    ...
                                    {currentTranscript && isListening && (
                                        <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} style={{ color: '#fef08a', fontSize: '0.85rem', fontStyle: 'italic', background: 'rgba(234, 179, 8, 0.1)', padding: '8px 12px', borderRadius: '8px', border: '1px solid rgba(234, 179, 8, 0.2)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'normal', wordBreak: 'break-word', boxShadow: '0 4px 15px rgba(0,0,0,0.3)' }}>{"\""}{currentTranscript}...{"\""}</motion.div>
                                    )}
