...
export default function EarningsDashboard() {
    ...
    if (loading) return <div>{t('dashboard.finance.loading')}</div>;
        ...
        <div style={{ display: 'grid', gap: '2rem' }}>
            ...
            <div style={{
                ...
                <div style={{ marginBottom: '2rem' }}>
                    <h3 style={{ fontSize: '1.2rem', fontWeight: 800, color: '#1a1a1b' }}>{t('dashboard.finance.salesPerformance')}</h3>
                    <p style={{ fontSize: '0.85rem', color: '#888' }}>{t('dashboard.finance.revenueVolume')}</p>
                </div>
            ...
            {summary.pendingFees > 0 && (
                ...
                >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                        ...
                        <div>
                            <div style={{ fontWeight: 800, color: '#9a3412', fontSize: '0.9rem' }}>{t('dashboard.finance.pendingFees')}</div>
                            <div style={{ fontSize: '0.8rem', color: '#c2410c' }}>{t('dashboard.finance.pendingFeesNotice').replace('{amount}', summary.pendingFees.toFixed(2))}</div>
                        </div>
                    </div>
                </motion.div>
            )}
            ...
            {user?.plan === 'free' && (
                ...
                >
                    ...
                    >
                        {t('dashboard.finance.viewPlans')} <ArrowUpRight size={16} />
                    </button>
                </motion.div>
            )}
            ...
            <div style={{ background: '#fff', padding: '2rem', borderRadius: '24px', boxShadow: '0 4px 20px rgba(0,0,0,0.05)' }}>
                <h3 style={{ fontSize: '1.2rem', fontWeight: 700, marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <Clock size={20} color="#D4AF37" /> {t('dashboard.finance.recentTransactions')}
                </h3>

                <div style={{ display: 'grid', gap: '1rem' }}>
                    {transactions.length === 0 ? (
                        <p style={{ color: '#999', textAlign: 'center', padding: '2rem' }}>{t('dashboard.finance.noTransactions')}</p>
                    ) : (
