    ...
    return (
        <div style={{ padding: '20px' }}>
            <div style={{ 
                ...
                <div>
                    <h2 style={{ fontSize: '1.8rem', fontWeight: 800, color: '#333' }}>Gestão de Livros & Marketplace</h2>
                    <p style={{ color: '#666' }}>Aprova submissões da comunidade ou adiciona novos links de afiliados.</p>
                </div>
                <button
                    onClick={() => {
                        setEditingBook(null);
                        setFormData({ title: '', author: '', description: '', coverImage: '', affiliateLink: '', pdfUrl: '', sellerPaypalEmail: '', price: '', category: 'Empreendedorismo', rating: 5, isActive: true, status: 'approved' });
                        setIsModalOpen(true);
                    }}
                    style={{
                        padding: '12px 24px',
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '30px' }}>
                ...
                <div style={{ background: '#fff', padding: '20px', borderRadius: '16px', border: '1px solid #eee' }}>
                    <div style={{ color: '#666', fontSize: '0.8rem', fontWeight: 700, textTransform: 'uppercase', marginBottom: '10px' }}>Submissões Pendentes</div>
                    <div style={{ fontSize: '1.5rem', fontWeight: 900, color: '#eab308' }}>{books.filter(b => b.status === 'pending').length}</div>
                </div>
                <div style={{ background: '#fff', padding: '20px', borderRadius: '16px', border: '1px solid #eee' }}>
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            <thead>
                                <tr style={{ background: '#f8f9fa' }}>
                                    ...
                                    <th style={{ padding: '15px', textAlign: 'left', fontSize: '0.75rem', fontWeight: 800, color: '#888' }}>TIPO</th>
                                    <th style={{ padding: '15px', textAlign: 'left', fontSize: '0.75rem', fontWeight: 800, color: '#888' }}>PREÇO</th>
                            ...
                            <tbody>
                                {filteredBooks.map(book => (
                                    <tr key={book._id} style={{ borderBottom: '1px solid #f8f9fa' }}>
                                        ...
                                        <td style={{ padding: '15px' }}>
                                            <span style={{ fontSize: '0.65rem', fontWeight: 800, padding: '4px 8px', background: '#f0f0f0', borderRadius: '6px' }}>{book.category}</span>
                                        </td>
                                        <td style={{ padding: '15px' }}>
                                            {book.isUserSubmission ? (
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '0.7rem', color: '#6366f1', fontWeight: 700 }}>
                                                    <FileText size={12} /> Comunitário
                                                </div>
                                            ) : (
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '0.7rem', color: '#e67e22', fontWeight: 700 }}>
                                                    <ShoppingBag size={12} /> Afiliado
                                                </div>
                                            )}
                                        </td>
                                        <td style={{ padding: '15px' }}>
                                            <div style={{ fontWeight: 800, color: book.price ? '#22c55e' : '#888' }}>
                                            ...
                                            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                                                {book.status === 'pending' && (
                                                    <div style={{ display: 'flex', gap: '5px' }}>
                                                        <button onClick={() => handleUpdateStatus(book._id!, 'approved')} style={{ background: '#22c55e', color: '#fff', border: 'none', padding: '6px 10px', borderRadius: '8px', fontSize: '0.7rem', fontWeight: 800, cursor: 'pointer' }}>✓</button>
                                                        <button onClick={() => handleUpdateStatus(book._id!, 'rejected')} style={{ background: '#ef4444', color: '#fff', border: 'none', padding: '6px 10px', borderRadius: '8px', fontSize: '0.7rem', fontWeight: 800, cursor: 'pointer' }}>✕</button>
                                                    </div>
                                                )}
                                                <button onClick={() => openEdit(book)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#6366f1' }} title="Editar"><Edit size={18} /></button>
                                                <button onClick={() => handleDelete(book._id!)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ef4444' }} title="Eliminar"><Trash2 size={18} /></button>
                                                {book.pdfUrl && <a href={book.pdfUrl} target="_blank" rel="noreferrer" style={{ color: '#e74c3c' }} title="Ver PDF"><FileText size={18} /></a>}
                                                {book.affiliateLink && <a href={book.affiliateLink} target="_blank" rel="noreferrer" style={{ color: '#888' }} title="Ver Link Amazon"><ExternalLink size={18} /></a>}
                                            </div>
                                        </td>
                                    </tr>
                                ))}
                    ...
                    <div style={{
                        ...
                        >
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '30px' }}>
                                <div>
                                    <h3 style={{ fontSize: '1.5rem', fontWeight: 800 }}>{editingBook ? 'Editar Livro' : 'Novo Livro'}</h3>
                                    <p style={{ color: '#666', fontSize: '0.85rem' }}>{formData.isUserSubmission ? 'Submissão de Utilizador' : 'Livro Próprio / Afiliado'}</p>
                                </div>
                            ...
                            <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                                ...
                                <div style={{ gridColumn: 'span 2', padding: '15px', background: '#f8f9fa', borderRadius: '12px', border: '1px solid #eee' }}>
                                    <h4 style={{ fontSize: '0.9rem', fontWeight: 800, marginBottom: '15px', color: '#333' }}>Configurações de Marketplace / Afiliado</h4>
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                                        <div>
                                            <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 800, color: '#666', marginBottom: '8px' }}>LINK AMAZON / AFILIADO</label>
                                            <input type="url" value={formData.affiliateLink} onChange={e => setFormData({...formData, affiliateLink: e.target.value})} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #ddd' }} />
                                        </div>
                                        <div>
                                            <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 800, color: '#666', marginBottom: '8px' }}>URL DO PDF (OBRA)</label>
                                            <input type="url" value={formData.pdfUrl} onChange={e => setFormData({...formData, pdfUrl: e.target.value})} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #ddd' }} />
                                        </div>
                                        <div>
                                            <label style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '0.75rem', fontWeight: 800, color: '#22c55e', marginBottom: '8px' }}>
                                                <DollarSign size={14} /> PREÇO (USD)
                                            </label>
                                            <input type="number" step="0.01" value={formData.price} onChange={e => setFormData({...formData, price: e.target.value})} placeholder="0.00" style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #ddd' }} />
                                        </div>
                                        <div>
                                            <label style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '0.75rem', fontWeight: 800, color: '#1A73E8', marginBottom: '8px' }}>
                                                <CreditCard size={14} /> E-MAIL PAYPAL (VENDEDOR)
                                            </label>
                                            <input type="email" value={formData.sellerPaypalEmail} onChange={e => setFormData({...formData, sellerPaypalEmail: e.target.value})} placeholder="exemplo@vendedor.com" style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #ddd' }} />
                                        </div>
                                    </div>
                                </div>

                                <div>
                                    <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 700, marginBottom: '8px' }}>Estado de Aprovação</label>
                                ...
                                <div style={{ gridColumn: 'span 2' }}>
                                    <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 700, marginBottom: '8px' }}>Resumo / Descrição</label>
                                    <textarea value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1px solid #ddd', height: '80px', resize: 'none' }} />
                                </div>
