...
export default function MentorLessonsPage() {
    ...
    const fetchLearnLessons = async () => {
        ...
        try {
            ...
            const progressMap = new Map(progressData.progress.map((p: any) => [p.lesson._id || p.lesson, p]));
            ...
            const favSet = new Set(favData.map((l: any) => l._id));
    ...
    const generateCertificate = async () => {
        try {
            ...
            const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/certificates/generate`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
                        ...
                        >
                            ...
                            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                                ...
                                <div>
                                    ...
                                    <select value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value as any })} style={{ width: '100%', padding: '12px', borderRadius: '12px', border: '1px solid #e0e0e0' }}>
                                        <option value="basico">Básico</option>
                                        <option value="intermediario">Intermediário</option>
                                        <option value="avancado">Avançado</option>
                                    </select>
...
export default function MentorLessonsPage() {
    ...
    const openLesson = async (lesson: Lesson) => {
        ...
        try {
            const token = document.cookie.split('; ').find(row => row.startsWith('token='))?.split('=')[1];
            await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/lessons/${lesson._id}`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
        } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const url = editingLesson
                ? `${process.env.NEXT_PUBLIC_API_URL}/api/lessons/${editingLesson._id}`
                : `${process.env.NEXT_PUBLIC_API_URL}/api/lessons`; // POST uses base path now
    ...
    if (loading && lessons.length === 0 && manageLessons.length === 0) {
        return (
            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '60vh' }}>
                <div className="animate-spin" style={{ width: '40px', height: '40px', border: '4px solid #f3f3f3', borderTop: '4px solid #D4AF37', borderRadius: '50%' }}></div>
            </div>
            ...
            <AnimatePresence>
                {selectedLesson && (
                    <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '2rem' }}
                        onClick={closeLesson}
                    >
                        <motion.div
                            initial={{ scale: 0.9 }}
                            ...
                            exit={{ scale: 0.9 }}
                            onClick={(e) => e.stopPropagation()}
                            style={{ background: 'white', borderRadius: '24px', maxWidth: '1000px', width: '100%', maxHeight: '90vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}
                        >
                            <div style={{ padding: '1.5rem', borderBottom: '1px solid #f0f0f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                ...
                                <button onClick={closeLesson} style={{ border: 'none', background: 'transparent', cursor: 'pointer' }}><X size={24} /></button>
                            </div>
                            <div style={{ flex: 1, background: '#000' }}>
                                <video src={selectedLesson.videoUrl} controls autoPlay style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                            </div>
                        </motion.div>
                    </motion.div>
                )}
