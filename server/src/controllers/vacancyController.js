...
exports.deleteApplication = async (req, res) => {
    try {
        const { id } = req.params;
        const application = await JobApplication.findById(id).populate('vacancyId');
        ...
        if (req.user.role !== 'admin' && req.user.role !== 'SuperAdmin' && vacancy.createdBy?.toString() !== req.user.id) {
            return res.status(403).json({ message: 'Não tem permissão para remover esta candidatura.' });
        }

        await JobApplication.findByIdAndDelete(id);
        res.status(200).json({ success: true, message: 'Candidatura removida com sucesso' });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
};
